...
activities (\emph{tfList} to be assigned to the \emph{measObj}). For running CARNIVAL we have to access one element at the time from the \emph{weightObj} and \emph{tfList}. We use all this as inputs to run the CARNIVAL analysis
through the \emph{runCARNIVAL} function.
...
\caption{Solution network of the real-case study}
\label{fig:realCase}
\end{figure*}
