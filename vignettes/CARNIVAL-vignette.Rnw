...
\subsection{ILP solvers}\label{sec:solvers}
...
4. A small toy example where measurements for multiple timepoints are available
(\emph{CARNIVAL-mulT})
...
Now we show an example about how we can run CARNIVAL on a real case-study. We
will show step-by-step how we can build a prior knowledge of signalling and a
DoRothEA regulon list from ~\emph{OmniPath}~\cite{OmniPath} (see
\url{https://github.com/saezlab/OmnipathR} for the R-package). Next we show how
we can esti mate pathway activities via PROGENy (see
\url{https://github.com/saezlab/progeny} for the R-package). Next we will
demonstrate how we can use the \emph{viper}~\cite{viper} (see
\url{https://www.bioconductor.org/packages/release/bioc/html/viper.html} for the
R-package). Finally we will show how we can use CARNIVAL to combine all this
information in order to infer the causal regulatory network.

Through \emph{OmnipathR} package, we build the network object needed for
contextualizing the signalling network with CARNIVAL.
...
For the analysis we consider the expression data example from the \emph{progeny}
R-package (see \url{https://github.com/saezlab/progeny}). We estimate normalized
(from -1 to 1) pathway activity scores via the \emph{progeny} function for the
first sample and then we assign the inferred activities as node weights to the
progeny members from the \emph{progenyMembers} object loaded from
\emph{CARNIVAL} package. Users can estimate the pathway activities for any
sample they wish (see documentation of \emph{assignPROGENyScores()} function)
...
Next we can retrieve regulons from OmniPath and create the regulon table. From
there we obtain the \emph{viper} regulon list through the
\emph{createRegulonList} function (see documentation for more details).
...
From here we can estimate the TF activities and generate the continuos
measurement inputs for CARNIVAL.
...
So far we have generated the prior knowledge network which will be used to
contextualize the regulatory signalling network (\emph{netObj}) as well as a
list object containing the progeny weights (\emph{weightObj}) and the TF
activities (\emph{tfList} to be assigned to the \emph{measObj}). For running
CARNIVAL we have to access one element at the time from the \emph{weightObj} and
\emph{tfList}. We use all this as inputs to run the CARNIVAL analysis through
the \emph{runCARNIVAL} function. Since the input targets are not known, we
perform the \emph{invCARNIVAL} analysis.
...
The solution network of this small real-case application is as shown in
\ref{fig:real_case}.

\begin{figure*}
\centerline{\includegraphics[width=\textwidth]{real_case.jpg}}
\caption{Solution for the real case example}
\label{fig:real_case}
\end{figure*}

\subsection{Multiple time point toy example}
In case data with measurements of node activities at multiple time points is
available, CARNIVAL-mulT can use the additional information to refine the
network. To run CARNIVAL-mulT, the additional time points are added as
new rows into \emph{measObj}. The first row corresponds to the first time
point. The \emph{inputObj} needs to have the same number of rows as the
\emph{measObj} since the time points in the \emph{measObj} are mapped 1:1
to the time points in the \emph{inputObj}.

At the moment, CARNIVAL-mulT requires \emph{cplex} to run, see
section~\ref{sec:solvers} for more information on how to run \emph{cplex}.
<<eval=FALSE, pgf=TRUE, echo=TRUE>>=
library(CARNIVAL) # load CARNIVAL library

load(file = system.file("toy_measurements_ex3.RData",
                        package="CARNIVAL"))
load(file = system.file("toy_network_ex3.RData",
                        package="CARNIVAL"))
load(file = system.file("toy_input_ex3.RData",
                        package="CARNIVAL"))

# Change to the location of your cplex executable
solver_path <-
  "C:/Program Files/IBM/ILOG/CPLEX_Studio1210/cplex/bin/x64_win64/cplex.exe"

result = runCARNIVAL(inputObj = toy_input_ex3,
                     measObj = toy_measurements_ex3,
                     netObj = toy_network_ex3,
                     mulT = TRUE,
                     solver = "cplex",
...
print(result$weightedSIF)
@

The current implementation of CARNIVAL-mulT deals with the time points in
a qualitative way, meaning that it works on the information that time has
passed between the measurements but does not take into account how much
time has passed.

Please take this into consideration when looking at processes
that work on different time scales. There it might be better to run CARNIVAL
individually for single measurements and compare the resulting networks
afterwards.

\begin{figure*}
\centerline{\includegraphics[width=0.35\textwidth]{mulT.png}}
\caption{Solution for the multiple time point test case.}
\label{fig:mulT}
\end{figure*}
