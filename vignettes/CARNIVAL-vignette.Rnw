...
## Run PROGENy
pathway_scores<-runPROGENy(df_genenames,weight_matrix, z_scores = FALSE)
@
...
information (i.e. inputFile = NULL, inverseCR=TRUE). Note that the pathway 
scores from PROGENy is highly recommended to be included once running the 
Inverse CARNIVAL pipeline. Here we present Example 3 as a case study for the 
Inverse CARNIVAL pipeline (to investigate downstream toxicity effects of 
...
Then we perform the analysis by running CARNIVAL with the CBC solver:
...
The \emph{experimental_conditions} variable will then indicate which of the
experimental conditions should be considered for the analysis:
