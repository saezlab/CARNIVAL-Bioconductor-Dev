...
For Mac OS:
\emph{~/Applications/IBM/ILOG/CPLEX_Studio129/cplex/bin/x86-64_osx/cplex}

For Linux:
\emph{/opt/ibm/ILOG/CPLEX_Studio129/cplex/bin/x86-64_linux/cplex}

For Windows:
\emph{C:/Program Files/IBM/ILOG/CPLEX_Studio129/cplex/bin/x64_win64/cplex.exe}
...
pre-process input descriptions, run optimisation and export results as a list 
and network figures. Three built-in \emph{CARNIVAL} examples are also supplied 
as case studies for users. Moreover processed gene expression values (besides 
differential values) from RNA-seq can also be used as an input to 
\emph{CARNIVAL} but examples are not provided in this vignette.
...
The main function of the \emph{CARNIVAL} package is \emph{runCARNIVAL}. This 
function runs the pipeline by using the user-provided list of inputs or run 
CARNIVAL built-in examples. These inputs are:

- \emph{solverPath}: Path to executable cbc/cplex file - default set to NULL, in
...
- \emph{solver}: Solver to use: \emph{lpSolve}, \emph{cplex} or \emph{cbc} 
(Default set to \emph{lpSolve}).
...
solution comparing within the pool of accepted solution (fraction; Default set 
to 0.0001)
