...
The \emph{Ex2_measurements_SBV_EGF} object is the same as table generated from the \emph{generate_measfile} function after estimating the Transcription Factor (TF) activites via \emph{DoRothEA}~\cite{dorothea} analysis (see \emph{runDoRothEA} function) as described above. The \emph{Ex2_weights_SBV_EGF} are the normalized estimated pathway activities through \emph{PROGENy}~\cite{progeny} (sign indicating the direction of the regulation) (see \emph{runPROGENy}). The \emph{Ex2_inputs_SBV_EGF} represents the target of the perturbation (activation of EGF on this case). The \emph{Ex2_network_SBV_Omnipath} represents the signed and directed prior knowledge network. The proteins on this case are mapped as \emph{Uniprot ID's}.
...
<<eval=FALSE, pgf=TRUE, echo=TRUE>>=
result = runCARNIVAL(solverPath=solverPath, netObj = Ex2_network_SBV_Omnipath, 
                     measObj = Ex2_measurements_SBV_EGF, inputObj = Ex2_inputs_SBV_EGF, 
                     weightObj = Ex2_weights_SBV_EGF, timelimit = 3600, poolCap = 100,
                     poolReplace = 2, limitPop = 500, nodeID = "uniprot", 
                     UP2GS = TRUE, DOTfig = TRUE, solver = "cplex")
@
...
Since we set \emph{DOTfig=TRUE}, the analysis will generate the \emph{.dot} Figure \ref{fig:egf_network} of the network within the default \emph{DOTfig} directory (please use \emph{dir name} variable if you wish to save the network solution at a different directory and on a different name). This figure represents the combined solution network as in the \emph{weightedSIF}. Since the \emph{UP2GS=TRUE}, the generated network will have Gene ID's, otherwise it can be set to \emph{FALSE}. Otherwise if the user is not interested to generate any \emph{.dot} figure, it can as well keep the default \emph{DOTfig=FALSE} for the analysis.

\begin{center}
\begin{figure}[ht]
\includegraphics[height=22cm, width=16.5cm]{egf_network}
\caption{Network soltion - Example 1}
\end{figure}
\end{center}
