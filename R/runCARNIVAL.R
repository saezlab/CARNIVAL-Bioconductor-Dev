...
#'@param netObj Filename of the prior knowledge network - always required or 
#'set as NULL to run CARNIVAL built-in example
#'@param measObj Filename of the measurement file (here DoRothEA normalised 
#'enrichment scores) - always required or set as NULL to run CARNIVAL built-in 
#'example
#'@param inputObj Filename of the list for target of perturbation - optional 
#'or set as NULL to run CARNIVAL built-in example
#'@param weightObj Filename of the additional weight (here PROGENy pathway 
#'score) - optional or set as NULL to run CARNIVAL built-in example
...
#'@param parallelIdx1 First index number suitable for parallelisation (numeric 
#'- set to 1 by default)
#'@param parallelIdx2 Second index number suitable for parallelisation (numeric 
#'- set to 1 by default)
#'@param nodeID Define the input format of nodes in the network (either 
#''uniprot' or 'gene' symbol)
#'@param UP2GS For plotting: define if Uniprot ID will be converted to gene 
#'symbols for better readability (logical TRUE/FALSE)
