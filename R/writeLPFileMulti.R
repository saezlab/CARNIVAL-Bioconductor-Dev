...
#'
#' Same as the 'writeLPFile', but this one takes into cosideration the multiple 
...
writeLPFileMulti <- function(data = data, pknList = pknList, inputs = inputs, 
  ...
  oF <- 
    write_objective_function_all(dataMatrix = dataMatrix, 
                                 ...
                                 scores=scores, measWeights=measWeights)
  ...
  c6 <- write_constraints_6(variables = variables[1:(length(variables)-1)], 
                            dataMatrix = dataMatrix, inputs = inputs)
  c7 <- write_constraints_7(variables = variables[1:(length(variables)-1)], 
                            dataMatrix = dataMatrix, inputs = inputs)
  c8 <- write_constraints_8(variables = variables[1:(length(variables)-1)], 
                            inputs = inputs, pknList = pknList)
  c9 <- write_loop_constraints(variables = variables[1:(length(variables)-1)], 
