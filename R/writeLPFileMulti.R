#'\code{writeLPFileMulti}
#'
#' Same as the 'writeLPFile', but this one takes into cosideration the multiple experimental conditions.
#' 
...
#' @param experimental_conditions The experimental conditions to be accounted for
#' 
...
writeLPFileMulti <- function(data = data, pknList = pknList, inputs = inputs, alphaWeight=1, betaWeight=0.2, scores=scores, mipGAP=0.1, poolrelGAP=0.01, limitPop=100, poolCap=100, poolIntensity=0, poolReplace=2,timelimit=1800,measWeights=NULL, repIndex, condition="") {
  ...
  c2 <- write_constraints_2_all(variables = variables[1:(length(variables)-1)])
  c3 <- write_constraints_3_all(variables = variables[1:(length(variables)-1)])
  c4 <- write_constraints_4_all(variables = variables[1:(length(variables)-1)])
  c5 <- write_constraints_5_all(variables = variables[1:(length(variables)-1)])
  c6 <- write_constraints_6(variables = variables[1:(length(variables)-1)], dataMatrix = dataMatrix, inputs = inputs)
  c7 <- write_constraints_7(variables = variables[1:(length(variables)-1)], dataMatrix = dataMatrix, inputs = inputs)
  c8 <- write_constraints_8(variables = variables[1:(length(variables)-1)], inputs = inputs, pknList = pknList)
  c9 <- write_loop_constraints(variables = variables[1:(length(variables)-1)], pknList = pknList, inputs = inputs)
  c10 <- write_experimental_conditions_constraints(variables = variables)
  allC <- all_constraints_wLoop(c0 = c0, c1 = c1, c2 = c2, c3 = c3, c4 = c4, c5 = c5, c6 = c6, c7 = c7, c8 = c8, c9 = c(c9, c10))
